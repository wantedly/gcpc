...
module Gcpc
  class Subscriber
    ...
    private
      ...
      def fire_event(lifecycle_event)
        arr = @config[lifecycle_event]
        arr.each do |block|
          block.call
        end
      end
        ...
        @heartbeat_worker_thread = Thread.new do
          ...
          begin
            ...
            loop do
              ...
              next if (Time.now.to_i - last_fire_event_executed_at) < BEAT_INTERVAL
  
              fire_event(:beat)
              last_fire_event_executed_at = Time.now.to_i
            end
          ensure
